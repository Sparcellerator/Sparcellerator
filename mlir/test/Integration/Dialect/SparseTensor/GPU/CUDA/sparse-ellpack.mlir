...
#ELLPACK = #sparse_tensor.encoding<{ map = (d0, d1) -> (d0 : dense, d1 : ellpack) }>

module {
  // Compute matrix vector y = Ax
  func.func @matvec(%A: tensor<?x?xf64, #ELLPACK>, %x: tensor<?xf64>, %y_in: tensor<?xf64>) -> tensor<?xf64> {
    %y_out = linalg.matvec
      ins(%A, %x: tensor<?x?xf64, #ELLPACK>, tensor<?xf64>)
  ...
  func.func @main() {
    ...
    // Convert to a "sparse" m x n matrix A using ELLPACK format
    %A = sparse_tensor.convert %DA : tensor<1024x64xf64> to tensor<?x?xf64, #ELLPACK>

    // Initialize dense vector with n elements:
    //   (1, 2, 3, 4, ..., n)
    %d1 = tensor.dim %A, %c1 : tensor<?x?xf64, #ELLPACK>
    ...
    %d0 = tensor.dim %A, %c0 : tensor<?x?xf64, #ELLPACK>
    ...
    // Call the kernel.
    %0 = call @matvec(%A, %x, %y) : (tensor<?x?xf64, #ELLPACK>, tensor<?xf64>, tensor<?xf64>) -> tensor<?xf64>
    ...
    bufferization.dealloc_tensor %A : tensor<?x?xf64, #ELLPACK>
    return
  }
}
